...
#                 #* target : [20,H,W]
#                 p = pred[j]
#                 # t = target[j+1]#* target[0] is the 
#                 t = target[j]#* target[0] is the 
#                 # embed()

#                 num_pos = torch.sum(t == 1).float()
#                 num_neg = torch.sum(t == 0).float()
#                 num_total = num_neg + num_pos  # true total number
#                 pos_weight = (num_neg / num_pos).clamp(min=1, max=num_total)  # compute a pos_weight for each image #*  large if num_pos is small  otherwise small

#                 max_val = (-p).clamp(min=0)#* must be the 0 
...
class EdgeDetectionReweightedLosses(WeightedCrossEntropyWithLogits):
    ...
    def forward(self, *inputs):
        ...
        loss = super(EdgeDetectionReweightedLosses, self).forward(pre, target)
